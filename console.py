...
class HBNBCommand(cmd.Cmd):
    """ Contain this functionality for the HBNB console"""
    ...
    def do_update(self, args):
        ...
        # determine if key is presents
        if key not in storage.all():
            print("** no instance found **")
            return

        # first determine if kwarg or args
        if '{' in args[2] and '}' in args[2] and type(eval(args[2])) is dict:
            kwargs = eval(args[2])
            args = []  # reformat kwarg into list, ex: [<name>, <value>, ...]
    ...
    def help_update(self):
        """ Help informations for this update class """
        print("Update an object with new informations")
